...
colnames(victimas) <-  c(
   "Año"    ,  "Clave_Ent", "Entidad", "bien_jur_afectado", "tipo_delito","subtipo_delito","modalidad",
...
incidencia_objetivo <- incidencia_objetivo %>% 
  filter(subtipo_delito %in% subtipos)


delitos_subset <- as.data.frame(unique(incidencia_objetivo$tipo_delito)) 

incidencia_objetivo <- incidencia_objetivo %>% 
  mutate(delito_ab = ifelse(tipo_delito=="Homicidio doloso", "homicidio_dol",
                            ifelse(tipo_delito=="Feminicidio", "feminicidio",
                                   ifelse(tipo_delito=="Lesiones dolosas", "lesiones_dol",
                                          ifelse(tipo_delito=="Lesiones culposas", "lesiones_culp",
                                          ifelse(tipo_delito=="Aborto", "aborto",
                                                 ifelse(tipo_delito=="Secuestro", "secuestro",
                                                        ifelse(tipo_delito=="Tráfico de menores", "trafico_menores",
                                                               ifelse(tipo_delito=="Rapto", "rapto",
                                                                      ifelse(tipo_delito=="Abuso sexual", "abuso_sexual",
                                                                             ifelse(tipo_delito=="Acoso sexual", "acoso_sexual",
                                                                                    ifelse(tipo_delito=="Hostigamiento sexual", "hostigamiento_sexual",
                                                                                           ifelse(tipo_delito=="Violación simple", "violacion_simple",
                                                                                                  ifelse(tipo_delito=="Violación equiparada", "violacion_equiparada",
                                                                                                         ifelse(tipo_delito=="Incesto", "incesto",
                                                                                                                ifelse(tipo_delito=="Otros delitos que atentan contra la libertad y la seguridad sexual", "otros_seg_sex",
                                                                                                                       ifelse(tipo_delito=="Violencia familiar", "viol_familiar",
                                                                                                                              ifelse(tipo_delito=="Violencia de género en todas sus modalidades distinta a la violencia familiar", "viol_genero",
                                                                                                                                     ifelse(tipo_delito=="Incumplimiento de obligaciones de asistencia familiar", "asistencia_familiar", "ERROR:sin definir")))))))))))))))))))
...
incidencia_objetivo <- incidencia_objetivo %>% 
  group_by(Año, Clave_Ent, Entidad, bien_jur_afectado, 
           tipo_delito, Sexo, rango_edad, subtipo_delito,modalidad,  delito_ab) %>% 
  summarise_at(meses[1:12], sum, na.rm = TRUE)

# sumar por delito nacional

tot_mensual_nac <- incidencia_objetivo %>% 
  group_by(Año, tipo_delito) %>% 
  summarise_at(meses[1:12], sum, na.rm = TRUE)

#### incidencia historica

incidencia_historica <- incidencia_objetivo %>% 
  group_by(Año, Clave_Ent, Entidad, bien_jur_afectado, 
           tipo_delito, subtipo_delito, Sexo,  delito_ab) %>% 
  summarise_at(meses[1:12], sum, na.rm = TRUE)
...
incidencia_objetivo_t <-incidencia_historica  %>%
  pivot_longer(!c(Año, Clave_Ent, Entidad, bien_jur_afectado, 
                  tipo_delito, subtipo_delito, Sexo, delito_ab), names_to = "mes", values_to = "incidencia_mes")
...
#sumatoria por mes
incidencia_objetivo_t <- incidencia_objetivo_t %>%
  group_by(datetime, anual, mes_anual, Entidad,Clave_Ent, Sexo, delito_ab ) %>%
...
incidencia_objetivo_t <- incidencia_objetivo_t %>%
  ...
  group_by(Entidad,Clave_Ent, Sexo, delito_ab) %>%
...
indicador_mensual$eva_incidencia_m2 <- case_when(indicador_mensual$mes2_t_pob<=indicador_mensual$V5 & indicador_mensual$mes2_t_pob>indicador_mensual$V4 ~ 5, 
...
indicador_mensual <- indicador_mensual %>%
  select(1:18, eva_incidencia_tcmes2 )
indicador_mensual$fecha_pact <- today()


write.csv(indicador_mensual, file="indicador_victimas_mensual.csv", fileEncoding ="latin1")
...
incidencia_ac1 <- aggregate(total_mensual ~ period + Clave_Ent + Entidad + Sexo + delito_ab, data = incidencia_ac1, sum)
...
write.csv(indicador_anual, file="indicador_victimas_anual.csv", fileEncoding ="latin1")
