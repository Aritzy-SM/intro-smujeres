...
incidencia  <- read_excel("incidencia delictiva/datos/Estatal-Delitos-2015-2024_nov2024.xlsx")

victimas <- read_xlsx("incidencia delictiva/datos/Estatal-V¡ctimas-2015-2024_nov2024.xlsx")
  ...
  mutate(delito_ab = ifelse(delito=="Homicidio doloso", "homicidio_dol",
                            ifelse(delito=="Feminicidio", "feminicidio",
                                   ifelse(delito=="Lesiones dolosas", "lesiones_dol",
                                          ifelse(delito=="Aborto", "aborto",
                                                 ifelse(delito=="Secuestro", "secuestro",
                                                        ifelse(delito=="Tráfico de menores", "trafico_menores",
                                                               ifelse(delito=="Rapto", "rapto",
                                                                      ifelse(delito=="Abuso sexual", "abuso_sexual",
                                                                             ifelse(delito=="Acoso sexual", "acoso_sexual",
                                                                                    ifelse(delito=="Hostigamiento sexual", "hostigamiento_sexual",
                                                                                           ifelse(delito=="Violación simple", "violacion_simple",
                                                                                                  ifelse(delito=="Incesto", "incesto",
                                                        ifelse(delito=="Otros delitos que atentan contra la libertad y la seguridad sexual", "otros_seg_sex",
                                                               ifelse(delito=="Violencia familiar", "viol_familiar",
                                                                      ifelse(delito=="Violencia de género en todas sus modalidades distinta a la violencia familiar", "viol_genero",
                                                                             ifelse(delito=="Incumplimiento de obligaciones de asistencia familiar", "asistencia_familiar", "ERROR:sin definir")))))))))))))))))
...
#### ---------- importar poblacion para sacar tasas de cambio--------------- ###


pob_2020 <- read_csv("~/Documents/aritzy/mujeres/pob/iter_00_cpv2020/conjunto_de_datos/conjunto_de_datos_iter_00CSV20.csv")

names(pob_intercensal2015)
names(pob_intercensal2015)[1] <- "cve_mun"
names(pob_intercensal2015)[6] <- "pobtot"
...
subtipos <- c("Homicidio doloso", "Secuestro", "Extorsión", "Robo de vehículo automotor",
              ...
              "Violencia de género en todas sus modalidades distinta a la violencia familiar", "Robo a institución bancaria", "Robo de maquinaria", "Robo a casa habitación", "Daño a la propiedad"	)
select_delitos_todos <- incidencia_objetivo %>% 
  filter(`Subtipo de delito` %in% subtipos)
...
#   rowwise() %>% 
#   replace(is.na(.), 0) %>%
#   mutate(a1 = ifelse(Año==2018, sum(Diciembre), 
#                      ifelse(Año==2019, sum(Enero, Febrero, Marzo, Abril, Mayo, Junio, Julio, Agosto, Septiembre, Octubre, Noviembre), 0)),
#          a2 = ifelse(Año==2019, sum(Diciembre), 
#                      ifelse(Año==2020, sum(Enero, Febrero, Marzo, Abril, Mayo, Junio, Julio, Agosto, Septiembre, Octubre, Noviembre), 0)),
#          semestre1 = ifelse(Año==2019, sum( Junio, Julio, Agosto, Septiembre, Octubre, Noviembre),0),
#          semestre2 = ifelse(Año==2020, sum( Junio, Julio, Agosto, Septiembre, Octubre, Noviembre),0),
#          mes1= ifelse(Año==2020 , sum(Octubre), 0),
#          mes2= ifelse(Año==2020 , sum(Noviembre),0))
...
#### agregar por estado, delito###
names(delitos_indicadores)
delitos_indicadores <- delitos_indicadores %>%
  select((2:7), (20:21)) %>% 
  group_by(Clave_Ent, Entidad, `Bien jurídico afectado`, 
           `Tipo de delito`, delito, delito_ab) %>% 
  summarise_all(list(sum))

delitos_indicadores$cve_ent <- str_pad(delitos_indicadores$Clave_Ent, 2, pad = 0)

base_pob <- aggregate(pobtot ~ ent + nom_ent, data = pob_intercensal2015, sum)

names(base_pob)[1] <- "cve_ent"
names(base_pob)[2] <- "Entidad"

base_pob <- base_pob %>% 
  select(1, 3)

delitos_indicadores <- left_join(delitos_indicadores, base_pob, by = c("cve_ent"))

delitos_indicadores <- delitos_indicadores %>% 
  select(9, 2:8, 10)

delitos_indicadores <- delitos_indicadores %>% 
  mutate(a1_t_pob = round((a1/pobtot)*100000, 2),
